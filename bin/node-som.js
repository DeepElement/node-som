...
som.prototype.classify = function(a) {
  ...
  if (!this.statistics[dMin]) {
    for (this.statistics[dMin] = {c:0, p:0, i:[]}, b = 0;b <= this.vecLen - 1;b++) {
      this.statistics[dMin].i.push({min:Number.MAX_VALUE, max:Number.MIN_VALUE});
    }
  }
  this.statistics[dMin].c++;
  for (b = 0;b <= this.vecLen - 1;b++) {
    var c = a[b], d = this.statistics[dMin].i[b];
    d.min > c && (d.min = c);
    d.max < c && (d.max = c);
  }
  a = 0;
